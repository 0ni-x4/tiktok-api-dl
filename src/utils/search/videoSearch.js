 ...
 */
export const SearchVideo = async (keyword, cookie, page = 1, proxy) => new Promise(async (resolve) => {
    if (!cookie) {
        return resolve({
            status: "error",
...
const requestVideoSearch = async (keyword, page, cookie, proxy) => {
    return retry(async (bail, attempt) => {
        try {
            const { data } = await Axios(_tiktokSearchVideoFull(_videoSearchParams(keyword, page)), {
                method: "GET",
                headers: {
