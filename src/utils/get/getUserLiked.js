...
export const getUserLiked = (username, cookie, proxy, postLimit) => new Promise((resolve) => {
    if (!cookie) {
        return {
            status: "error",
...
const requestUserLiked = async (id, secUid, cookie, postLimit, proxy) => {
    ...
    const url = new URL(_tiktokGetUserLiked(_getUserLikedParams(id, secUid, postLimit)));
