...
export const _getUserLikedParams = (id, secUid, count) => {
    let cursor = 0;
    if (count > 50) {
...
export const _xttParams = (secUid, cursor, count) => {
    return qs.stringify({
        aid: "1988",
        cookie_enabled: true,
        screen_width: 0,
...
export const _getCommentsParams = (id, count) => {
    let cursor = 0;
    if (count > 50) {
        for (let i = 1; i < count; i++) {
            cursor += 50;
...
/** Search Params */
export const _userSearchParams = (keyword, page, xbogus) => {
    let cursor = 0;
    if (page > 1) {
...
export const _liveSearchParams = (keyword, page) => {
    let cursor = 0;
    if (page > 1) {
        for (let i = 1; i < page; i++) {
            cursor += 12;
...
export const _videoSearchParams = (keyword, page) => {
    let cursor = 0;
    if (page > 1) {
        for (let i = 1; i < page; i++) {
            cursor += 12;
...
/** Downloader Params */
export const _tiktokApiParams = (args) => {
    return new URLSearchParams({
        ...args,
        version_name: "1.1.9",
...
export const _getCollectionParams = (collectionId, page = 1, count = 5) => {
    let cursor = 0;
    if (page > 0) {
        cursor = (page - 1) * count;
    }
...
export const _getPlaylistParams = (playlistId, page = 1, 
/**
 * @max 20
 * @default 5
 */
